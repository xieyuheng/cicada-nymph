...
** note name-stack
   * (define-*) push
     (undo) pop
** note name-record
   * *name-stack* contain name-record
   * structure
     | name-record | old-jo |
     |             | name   |
     |             | new-jo |
** *name-stack*
   ...
   *name-stack,size*
   *name-stack,unit* mul allocate-memory
   : *name-stack*
   ; define-variable,with-tos
...
** name,record-jo
   ...
   : name,record-jo
     << jo, name -- >>
     dup name,get-jo
     *name-stack,pointer* name-record,set-old-jo
     dup2 name,set-jo
     *name-stack,pointer* name-record,set-name
     *name-stack,pointer* name-record,set-new-jo
     *name-stack,unit* address *name-stack,pointer* add-set
     end
   ; define-function
   #+end_src
** jo,find-name
...
** name-hash-table,record-jo,by-link
   ...
   ; define-function
   #+end_src
** name-stack,delete-last-record
   #+begin_src cicada-nymph :tangle core.cn
   : name-stack,delete-last-record
     << -- >>
     *name-stack,unit* address *name-stack,pointer* sub-set
     end
   ; define-function
   #+end_src
* undo
...
*** restore,name-stack
    ...
    : restore,name-stack
      ...
      *jo-size* 3 mul add get >:old-record
      :old-record *name-stack,pointer*
      restore,name-stack,set-jo
      :old-record address *name-stack,pointer* set
      end
    ; define-function
    #+end_src
*** 記 語法棧的備份
...
** <report> syntax
   ...
   ; define-function
   #+end_src
** <name-stack> list-by-jo-predicate
   * 最後寫到珠典裏的 被最先打印出來
   #+begin_src cicada-nymph :tangle core.cn
   : <name-stack> list-by-jo-predicate,loop
     << jo-predicate, counter, cursor -- >>
     dup *name-stack,pointer* equal? if
       drop
       drop
       drop
       end
...
** <report> name-stack
   * different types of words in link
   ...
   : <report> name-stack
     << -- >>
     "* all primitive-function in name-stack :" .s .l
     jo primitive-function-jo?
     <name-stack> list-by-jo-predicate

     "* all function in name-stack :" .s .l
     jo function-jo?
     <name-stack> list-by-jo-predicate

     "* all exception in name-stack :" .s .l
     jo exception-jo?
     <name-stack> list-by-jo-predicate

     "* all variable in name-stack :" .s .l
     jo variable-jo?
     <name-stack> list-by-jo-predicate
     end
   ; define-function
