...
** more,rule-set,make-jojo
   ...
   : more,rule-set,make-jojo
     ...
     *rule-set,make-jojo*
     ...
     dup jo word:prepare-for?           jo syntax,prepare-for,make-jojo            rule,add
     dup jo word:exception-reset-stack? jo syntax,exception-reset-stack,make-jojo  rule,add
...
*** <basic-REPL-syntax> if
    ...
    ; define-function
    #+end_src
*** rule,add to *rule-set,basic-REPL*
    #+begin_src cicada-nymph :tangle core.cn
    *rule-set,basic-REPL*
...
*** 記
    * 注意這裏形成動態調用的方式
      這在於
      (<pre-module-syntax> import) 必須在 (import) 之後定義
      所以在 (import) 中
      對 (<pre-module-syntax> import) 的調用必須是間接的
      這是通過 一個全局變量
      還有 下面的兩個標記語法變更的函數 來實現的
    * 語法變更不是 入 新 *rule-set* 到 語法棧
      而是把 一個 新 *rule-set* 中的全部規則
      複製到 <basic-REPL> *rule-set* 中
      這樣就是算是 *rule-set* 之間的繼承
*** <pre-module> *rule-set*
    #+begin_src cicada-nymph :tangle core.cn
...
*** ><>< <pre-module-syntax> import
    * import module and expose-naming
    ...
    : <pre-module-syntax> import,loop
      ...
      then
      ::word import
      ::word string->name <module-stack> find not if
...
*** <pre-module-syntax> unknow-meta-word
    #+begin_src cicada-nymph :tangle core.cn
    : <pre-module-syntax> unknow-meta-word,loop
      << -- >>
