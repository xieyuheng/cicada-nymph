...
* 在 (name-hash-table,save-jo) 中使用 share 與否 用 report-memory 查看 並沒有效果
  * 單純對 (name->string,share) 的測試 用 report-memory 查看 是有效果的
* update (!undo-make-jojo) for name-hash-table
...
* un-named function
  ...
  * cicada-language 還是必要的嘛 ?
    ...
    調和一下兩個語義的差異
* 匿名的 jojo 前面是包含 詮釋者的
* always do not use jo quote, but to use jojo quote i.e. unnamed function :maybe:
  * remove the jo quote
  * 因爲
    否則 帶有 <title> 的 jo 就不好辦了
...
** test
   * set *name-hash-table,size* to a small number [for example 13]
   ...
   : test,name-hash-table,share
     << -- >>
     "a-000" string->name,share . .l
     "a-111" string->name,share . .l
     "a-222" string->name,share . .l
     "a-333" string->name,share . .l
     "a-444" string->name,share . .l
     "a-555" string->name,share . .l
     "a-666" string->name,share . .l
     "a-777" string->name,share . .l
     "a-888" string->name,share . .l
     "a-999" string->name,share . .l
     "b-000" string->name,share . .l
     "b-111" string->name,share . .l
     "b-222" string->name,share . .l
     "b-333" string->name,share . .l
     "b-444" string->name,share . .l
     "b-555" string->name,share . .l
     "b-666" string->name,share . .l
     "b-777" string->name,share . .l
     "b-888" string->name,share . .l
     "b-999" string->name,share . .l
     end
   ; define-function
...
** test
   ...
   ; define-function,with-title
   "<test-title> test-name-1" execute-word
   "<test-title>" "test-name-1" name-hash-table,find-jo,with-title drop execute-jo
   <test-title> link->jo execute-jo
   ...
   ; define-function,with-title
   "<test-title> test-name-2" execute-word
   "<test-title>" "test-name-2" name-hash-table,find-jo,with-title drop execute-jo
   <test-title> link->jo execute-jo
   ...
   ; define-function,with-title
   "<test-title> test-name-3" execute-word
   "<test-title>" "test-name-3" name-hash-table,find-jo,with-title drop execute-jo
   <test-title> link->jo execute-jo
   #+end_src
