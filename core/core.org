...
* todo
** 有了匯編器之後 所有的名字都應該被 name-hash-table 處理 這樣就沒有重複問題了 :也許:
** 重新實現關於局部變元的語法 使得可以在匿名函數中使用 修復 if 的問題 並增加語法
...
*** test
    * set *name-hash-table,size* to a small number [for example 13]
    ...
    name-hash-table,report
    #+end_src
*** name-hash-table,save-jo,by-link
    #+begin_src cicada-nymph :tangle core.cn
    : name-hash-table,save-jo,by-link
      << link -- >>
      >:link
...
*** name-hash-table,delete-jo
    ...
    : name-hash-table,delete-jo
      << jo -- >>
      >:jo
      :jo jo->name-string string->name >:name
      0 :name name,save-jo
      end
    ; define-function
    #+end_src
*** name-hash-table,delete-jo,by-link
    #+begin_src cicada-nymph :tangle core.cn
    : name-hash-table,delete-jo,by-link
      << link -- >>
      >:link
      :link link->name-string string->name >:name
      0 :name name,save-jo
      end
    ; define-function
    #+end_src
...
** note global states
   ...
   * *link*
   * name-hash-table
   * *backup-area,current-free-address*
   * syntax-stack
     and rule-set in it
** undo-point
*** note undo point
    ...
    * structure
      ...
      |            | syntax-backup-area  |
      |            | number of rule-sets |
    * interface function is not implemented
      because they will not be used very much
** *undo-stack*
*** note
    * I do not need the push & pop interface here
*** memory allocation
    #+begin_src cicada-nymph :tangle core.cn
    2048 *jo-size* mul
    : *undo-stack,size*
    ...
    *undo-stack,size* allocate-memory
    : *undo-stack*
    ; define-variable,with-tos
...
** *jo-backup-area*
   ...
   *jo-backup-area,size* allocate-memory
   : *jo-backup-area*
   ; define-variable,with-tos

   *jo-backup-area*
   : *jo-backup-area,current-free-address*
   ; define-variable,with-tos
   #+end_src
** *syntax-backup-area*
   #+begin_src cicada-nymph :tangle core.cn
...
*** 記
    * 將 undo-point 這個數據結構入 undo-stack
      並返回剛入棧的數據結構的地址
*** create-undo-point,copy-syntax,one-rule-set
    #+begin_src cicada-nymph :tangle core.cn
    : create-undo-point,copy-syntax,one-rule-set
      << rule-set -- >>
      >:rule-set
      ...
      *jo-size* div >:n
      ...
      :rule-set
      *syntax-backup-area,current-free-address* save
      *jo-size* address *syntax-backup-area,current-free-address* add-save
      ...
      *syntax-backup-area,current-free-address*
      :n n-save
      *jo-size* :n mul
      address *syntax-backup-area,current-free-address* add-save
      end
    ; define-function
    #+end_src
*** create-undo-point,copy-syntax
    #+begin_src cicada-nymph :tangle core.cn
    : create-undo-point,copy-syntax
      << cursor -- >>
      ...
      then
      dup fetch create-undo-point,copy-syntax,one-rule-set
      *jo-size* add
      loop
    ; define-function
    #+end_src
*** create-undo-point
    ...
    : create-undo-point
      ...
      *jo-size* 5 mul
      address *undo-stack,pointer* add-save

      *syntax-stack,address* create-undo-point,copy-syntax

      *syntax-stack,pointer*
      *syntax-stack,address* sub
      *jo-size* div
      *undo-stack,pointer* save
...
*** jojo-area
    ...
    : restore,jojo-area
      << undo-point -- >>
      fetch
      address *jojo-area,current-free-address* save
      end
    ; define-function
    #+end_src
*** string-area
    ...
    : restore,string-area
      << undo-point -- >>
      *jo-size* add fetch
      address *string-area,current-free-address* save
      end
    ; define-function
    #+end_src
*** link,remove-jo
    #+begin_src cicada-nymph :tangle core.cn
    : restore,link,remove-jo
      << old-link, link -- >>
      dup2 equal? if
        drop2
        end
      then
      dup name-hash-table,delete-jo,by-link
      link->next-link
      loop
    ; define-function
    #+end_src
*** link
    ...
    : restore,link
      << undo-point -- >>
      *jo-size* 2 mul add fetch >:old-link
      :old-link *link*
      restore,link,remove-jo
      :old-link address *link* save
      end
    ; define-function
    #+end_src
*** jo-backup-area,save-jo
    #+begin_src cicada-nymph :tangle core.cn
    : restore,jo-backup-area,save-jo
      << old-address, address -- >>
      dup2 equal? if
        drop2
        end
      then
      *jo-size* 2 mul sub
      dup 2 n-fetch name,save-jo
      loop
    ; define-function
    #+end_src
*** jo-backup-area
    #+begin_src cicada-nymph :tangle core.cn
    : restore,jo-backup-area
      << undo-point -- >>
      *jo-size* 3 mul add fetch >:old-address
      :old-address *jo-backup-area,current-free-address*
      restore,jo-backup-area,save-jo
      :old-address address *jo-backup-area,current-free-address* save
      end
    ; define-function
    #+end_src
*** 記 語法棧的備份
    ...
    * 語法棧中的每一個 規則集 在備份中的存在是
      ...
      並不必知道 jo 的語義
*** syntax-backup-area,save-syntax
    #+begin_src cicada-nymph :tangle core.cn
    : restore,syntax-backup-area,save-syntax
      ...
      :rule-set-copy fetch >:rule-set
      :rule-set-copy *jo-size* add fetch >:n

      :rule-set :cursor save
...
*** syntax-backup-area
    #+begin_src cicada-nymph :tangle core.cn
    : restore,syntax-backup-area
      << undo-point -- >>
      dup
      *jo-size* 4 mul add fetch >:old-address
      *jo-size* 5 mul add fetch >:number-of-rule-sets

      *syntax-stack,address*
      :old-address
      :number-of-rule-sets
      restore,syntax-backup-area,save-syntax >:cursor

      :cursor
      address *syntax-stack,pointer* save

      :old-address address *syntax-backup-area,current-free-address* save
      end
    ; define-function
    #+end_src
*** undo-stack
    ...
    : restore,undo-stack
      << undo-point -- >>
      address *undo-stack,pointer* save
      end
    ; define-function
    #+end_src
** undo
   ...
   : undo
     ...
     :undo-point restore,link
     :undo-point restore,jo-backup-area
...
*** init,name-hash-table,by-link
    ...
    : init,name-hash-table,by-link
      ...
      ::name-string name-hash-table,search swap >:name if
        ...
        then
      then
      :link name-hash-table,save-jo,by-link
      :link link->next-link
      loop
    ; define-function
    #+end_src
** define-function                    :redefine:
   ...
   : define-function
     ...
     *link* xx|swap|x
     ...
     ::name string->name >:name-index
     :name-index name,used-as-jo? if
       :name-index name,fetch-jo
       :name-index
       *jo-backup-area,current-free-address*
       2 n-save
       *jo-size* 2 mul
       address *jo-backup-area,current-free-address*
       add-save
     then
     :link name-hash-table,save-jo,by-link
...
*** circular-string-area,stay
    ...
    : circular-string-area,stay
      ...
      *circular-string-area,size* *circular-string-area* add
      greater-or-equal? if
        *circular-string-area*
        address *circular-string-area,current-free-address* save
      then
      tuck
      *circular-string-area,current-free-address*
      string->buffer!
...
** reset *lost-exception-REPL*
   #+begin_src cicada-nymph :tangle core.cn
   jo stack-REPL address *lost-exception-REPL* save
   #+end_src
** test
   #+begin_src cicada-nymph
...
*** 加載棧
    * 爲了實現 (need)
      meta-code 需要被做爲字符串而保存在 加載棧 中
      用以聞訊
      同時
      這些信息 也可以幫助實現詳細的 report
*** 沒有啓動時默認加載的文件
    * 沒有文件系統中的代碼 會被在啓動時默認加載
      用戶必須配置 (poi)
      然後利用 path-syntax 加載自己所需要的代碼
    * 我提供 命令行函數 來幫助在啓動時加載文件
*** 減載
    * 或者說 撤銷加載 (unload)
      它利用 (undo) 來實現
** *loading-stack*
