...
* variable must be able to store string
  * 更改定義 variable 的語法
    使得一個 variable 中可以保存多個值
    並且提供語法 [或函數]
    來以不同的方式 取出其中的全部值或部分值
  * 通過更改匯編代碼中的 explain$variable 來實現這一點
* name-hash-table
  * 讓 name 能夠在 cicada-nymph 和 cicada 中被同時使用
    讓 find 這個函數使用 name-hash-table
    也就是說 將有兩個 find 函數
    1. 一個通過單項鏈接的鏈表來搜索
    2. 一個通過 name-hash-table 來搜索
  * name-hash-table 本身的接口也應該更豐富
* todo
...
** init-syscall-number                :32bit:
   ...
   *jo-size* 4 equal? if
     ...
     183 : :syscall-number,getcwd:     ; define-variable
     12  : :syscall-number,chdir:      ; define-variable
   then
   #+end_src
** init-syscall-number                :64bit:
   ...
   *jo-size* 8 equal? if
     ...
     79  : :syscall-number,getcwd:     ; define-variable
     80  : :syscall-number,chdir:      ; define-variable
   then
   #+end_src
** error-code->string
   #+begin_src cicada-nymph :tangle core.cn
