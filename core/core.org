...
* since I have (current-defining-word) I can do a better version of <>
  (recur) and (loop) as two syntax for make-jojo
* loading-stack
* let functions about (poi) to use predicate [as I promised]
  * thus better info can be printed in the command-line-interface
* un-named function
  * 我發現我已經可以使用 map 了
    ...
    2. 使用 *circular-jo-area*
       要知道現在的效果是
       string 在 REPL 中出現時 就用 *circular-string-area*
       string 在 make-jojo 中出現時 就用 *name-string-area*
       這種設計對於函數可能也同樣適用
  * 我使用第二種方式來實現我需要的性狀
  ...
  * cicada-language 還是必要的嘛 ?
    ...
    調和一下兩個語義的差異
* todo
** better design of the command-line-interface
...
* note problem
** 在 name-string-area 中函數的姓名在被加載到 name-hash-table 時 被重新複製了一遍
...
** name,save-string
   ...
   * currently (name,save-string) is to
     copy string into name-string-area
...
** define-exception
   ...
   : define-exception
     << string[address, length] -- >>
     *current-free-address,name-string-area* xx|swap|x
     *current-free-address,jo-area* xx|swap|x
...
** syntax,double-quote,define-variable
   * in ASCII encode double-quote is 34
   ...
   : syntax,double-quote,define-variable
     ...
     syntax,double-quote,define-variable,loop
     >:cursor
     *current-free-address,name-string-area* add2 >:address
     :buffer
     :cursor :buffer sub dup >:length
     save-into,name-string-area
     :address
...
** report-memory
   ...
   : report-memory
     ...
     "  * free : " .s
          ...
          *un-initialized-memory*
          sub sub . .l
     "* *name-string-area*" .s .l
     "  * size : " .s
          *size,name-string-area*
          . .l
     "  * used : " .s
          *current-free-address,name-string-area*
          *name-string-area*
...
** command-line,poi
   ...
   * cn poi list
   ...
   : command-line,poi
     ...
     "  you can type  cn poi  to get some help info" .s .l
     "  good bye ^-^/" .s .l
     bye
   ; define-function
   #+end_src
