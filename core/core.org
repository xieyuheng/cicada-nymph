...
* un-named function
  ...
  * cicada-language 還是必要的嘛 ?
    ...
    調和一下兩個語義的差異
* 匿名的 jojo 前面是包含 詮釋者的
* always do not use jo quote, but to use jojo quote i.e. unnamed function
  * remove the jo quote
  * 因爲
...
** syntax,[if|else|then],make-jojo
   ...
   : syntax,else,make-jojo
     ...
     jo instruction,branch
        jojo-area,stay
     x|swap|xxx
     *current-free-address,jojo-area* xxxx|swap|x
     0 jojo-area,stay
...
** 記 實現
   * 每個 <title> 在全局的 jo-link 中
     ...
     其中保存一個 jo-link
   * 這個 jo-link 中的 jo 就是登記在 這個 <title> 下的 jo
...
** note
   * a title is a jo-link
     a name under title can be found
     by normal jo-link interface
...
** jo-link,report
   * different types of words in jo-link
   ...
   : jo-link,report
     ...
     *jo-link,core* jo primitive-function-jo?
     jo-link,list-by-jo-predicate
     "* all function in jo-link :" .s .l
     *jo-link,core* jo function-jo?
     jo-link,list-by-jo-predicate add
     "* all exception in jo-link :" .s .l
     *jo-link,core* jo exception-jo?
     jo-link,list-by-jo-predicate add
     "* all variable in jo-link :" .s .l
     *jo-link,core* jo variable-jo?
     jo-link,list-by-jo-predicate add
     "* totally : " .s
     .i .l
     end
