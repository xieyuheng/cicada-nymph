...
** 記 初始化
   * 只是把 link 中的信息全部都加進來
     注意
     是在實現了 模塊機制 之後 core 才被製作成模塊的
     在這之前 都只有由 name-hash-table 來實現的 全局命名空間而已
** note naming-stack
   * (define-*) push
     (undo) pop
** note name-record
   * *naming-stack* contain name-record
   * structure
     | name-record | old-jo |
     |             | name   |
     |             | new-jo |
** *naming-stack*
   ...
   *naming-stack,size*
   *naming-stack,unit* mul allocate-memory
   : *naming-stack*
   ; define-variable,with-tos

   *naming-stack*
   : *naming-stack,pointer*
   ; define-variable,with-tos
   #+end_src
...
** name,record-jo
   ...
   : name,record-jo
     << jo, name -- >>
     dup name,get-jo
     *naming-stack,pointer* name-record,set-old-jo
     dup2 name,set-jo
     *naming-stack,pointer* name-record,set-name
     *naming-stack,pointer* name-record,set-new-jo
     *naming-stack,unit* address *naming-stack,pointer* add-set
     end
   ; define-function
   #+end_src
** jo,find-name
   ...
   : jo,find-name,loop
     << jo, current-record
        -- name, true
        -- false >>
     dup *naming-stack* equal? if
       drop2
       false
       end
     then
     *naming-stack,unit* sub
     dup >:name-record
     over
...
** name-hash-table,record-jo,by-link
   ...
   ; define-function
   #+end_src
** naming-stack,delete-last-record
   #+begin_src cicada-nymph :tangle core.cn
   : naming-stack,delete-last-record
     << -- >>
     *naming-stack,unit* address *naming-stack,pointer* sub-set
     end
   ; define-function
   #+end_src
* basic-REPL
...
*** 模塊棧 之 使用分析
    * 需要記錄當前被加載的模塊名字
      函數定義時 要用其前綴以成名
    * 這個前綴以全局狀態傳遞給 define-function
      而不以函數的參數傳遞給 define-function
      這是爲了簡化 define-function 的接口
    * 這裏爲什麼需要棧
      而不單用一個全局變量
      其實
      使用單一的全局變量就夠了
...
*** 模塊棧 與 加載棧 辨析
    * 上面的棧其實是用來記錄 模塊的前綴的
      這些信息被 define-function 等等 所使用
    * 但是被成功加載的模塊
      有關其本身存在的信息並不記錄在上面這個棧中
      而是記錄在下面的 加載棧 中
    * 模塊棧 在加載過程中 爲 define-function 所用
      加載棧 負責記錄成功加載
*** 加載棧
    * 加載棧 記錄
...
*** 關於遞歸函數
    ...
    * 引入[import] 是遞歸函數
      ...
      而不等待 exception
*** 臨時命名棧
    * 臨時命名棧 [temporary naming-stack]
    * 其中所保存的命名全都不帶 模塊 前綴
      而利用這些命名去做定義
      每個定義都對應兩個命名
      一個帶有前綴的 記錄在 *naming-stack* 中
      一個不帶前綴的 記錄在 當前的 temporary naming-stack 中
      所以說
      temporary naming-stack 本身應該做爲一個全局的棧而存在
      記爲 *temporary-naming-stack*
*** ><>< 問題
    * 應該用兩種 命名棧
      其一 所有名字都有 模塊前綴
      其一 所有名字都沒有 模塊前綴
...
*** create-undo-point,copy-syntax
    ...
    : create-undo-point,copy-syntax
      ...
      then
      dup get create-undo-point,copy-syntax,one-rule-set
      *jo-size* add
      loop
    ; define-function
    #+end_src
*** create-undo-point
    ...
    : create-undo-point
      ...
      *jo-size* 5 mul
      address *undo-stack,pointer* add-set
...
*** restore,naming-stack,set-jo
    ...
    ; define-function
    #+end_src
*** restore,naming-stack
    #+begin_src cicada-nymph :tangle core.cn
    : restore,naming-stack
      << undo-point -- >>
      *jo-size* 3 mul add get >:old-record
      :old-record *naming-stack,pointer*
      restore,naming-stack,set-jo
      :old-record address *naming-stack,pointer* set
      end
    ; define-function
    #+end_src
*** 記 語法棧的備份
    ...
    * 語法棧中的每一個 規則集 在備份中的存在是
      ...
      並不必知道 jo 的語義
*** restore,syntax-backup-area,set-syntax
    #+begin_src cicada-nymph :tangle core.cn
    : restore,syntax-backup-area,set-syntax
...
*** restore,syntax-backup-area
    ...
    : restore,syntax-backup-area
      << undo-point -- >>
      dup
      *jo-size* 4 mul add get >:old-address
      *jo-size* 5 mul add get >:number-of-rule-sets

      *syntax-stack,address*
      :old-address
      :number-of-rule-sets
      restore,syntax-backup-area,set-syntax >:cursor

      :cursor
      address *syntax-stack,pointer* set

      :old-address address *syntax-backup-area,current-free-address* set
      end
    ; define-function
    #+end_src
*** restore,undo-stack
    ...
    : restore,undo-stack
      << undo-point -- >>
      address *undo-stack,pointer* set
      end
    ; define-function
    #+end_src
** undo
   ...
   : undo
     ...
     :undo-point restore,naming-stack
...
** <report> syntax
   ...
   ; define-function
   #+end_src
** <naming-stack> list-by-jo-predicate
   * 最後寫到珠典裏的 被最先打印出來
   #+begin_src cicada-nymph :tangle core.cn
   : <naming-stack> list-by-jo-predicate,loop
     << jo-predicate, counter, cursor -- >>
     dup *naming-stack,pointer* equal? if
       drop
       drop
       drop
       end
...
** <report> naming-stack
   * different types of words in link
   ...
   : <report> naming-stack
     << -- >>
     "* all primitive-function in naming-stack :" .s .l
     jo primitive-function-jo?
     <naming-stack> list-by-jo-predicate

     "* all function in naming-stack :" .s .l
     jo function-jo?
     <naming-stack> list-by-jo-predicate

     "* all exception in naming-stack :" .s .l
     jo exception-jo?
     <naming-stack> list-by-jo-predicate

     "* all variable in naming-stack :" .s .l
     jo variable-jo?
     <naming-stack> list-by-jo-predicate
     end
   ; define-function
