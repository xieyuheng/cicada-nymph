...
** test if & else & then
   ...
   #+end_src
** jo
   * NOT the same as the macro (address)
   #+begin_src cicada-nymph :tangle core.cn
   : jo
     << string[address, length] --
        string[address, length] >>
...
* basic-REPL                    :redefine:
  ...
  : basic-REPL
    << unknown -- unknown >>
    jo integer-string? jo string->integer add-syntax
    basic-REPL,loop
    jo integer-string? jo string->integer sub-syntax
    end
  ; define-function

  <<
  the following call is for
  we need a better REPL for the following code in core file
  >>
