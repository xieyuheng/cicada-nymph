...
* todo
** need more lib functions and better factoring for string processing
** 因實現蟬語而所需要的基本函數
...
** *home-directory*
   #+begin_src cicada-nymph :tangle core.cn
   : *home-directory*
     "HOME" find-environment-string-variable
   ; define-variable
   #+end_src
* working-directory
...
** change-working-directory
   ...
   : change-working-directory
     ...
     dup negative? if
       ...
       "  can not change the working directory into : " .s ::directory-path .s .l
       "  " .s error-code->string .s .l
       end
     then
     drop
...
** poi,ask
   ...
   : poi,ask
     ...
     *poi-directory,user* "/" ::name "/path"
     ...
     :buffer :length
     file,read
     :read-buffer swap >::the-path
     "* (poi,ask) " .s .l
     "  " .s ::name .s " : " .s ::the-path .s .l
     end
   ; define-function
   #+end_src
** poi,list
...
** test
   ...
   "test-cicada-play" poi,ask

   "test-cicada-play-2" "./play-2" poi,add
   "test-cicada-play-2" poi,ask
