...
** fix define in if-block
** 32-bits port for linux
** 32-bits port for windows
** [maybe] re generate instars
* ===================================
...
** __exit_with_TOS [linux]
   ...
   __exit_with_TOS:
      ...
      mov linux64_sys_n_rax, syscall_exit
      syscall
...
** write-byte [linux]
   ...
   define_primitive_function "write-byte", write_byte
      ...
      mov linux64_sys_n_rax, syscall_write
      ;;;;
...
** write-byte [windows]
   ...
   windows_function 5
      push 0
      sub rsp, 8*4
      mov windows64_fun_4_r9, _counter$write_byte
...
** read-line-from-stdin [linux]
   ...
   define_primitive_function "read-line-from-stdin", read_line_from_stdin
      ...
      mov linux64_sys_n_rax, syscall_read
      syscall
      ;; the return value
...
** read-line-from-stdin [windows]
   ...
   windows_function 5
      push 0
      sub rsp, 8*4
      mov windows64_fun_4_r9, _counter$read_byte
      mov windows64_fun_3_r8, max_input_length
      mov windows64_fun_2_rdx, buffer$read_byte
...
** open-file,to-read [linux]
   ...
   define_primitive_function "open-file,to-read", open_file__to_read
      ...
      ;;    [file handle] or [negate number on error] >>
      pop_argument_stack rcx
      ...
      mov linux64_sys_n_rax, syscall_open
      syscall
      push_argument_stack rax
      next
   #+end_src
** open-file,to-write [linux]
   #+begin_src fasm :tangle cicada-nymph.fasm
...
** close-file [linux]
   ...
   define_primitive_function "close-file", close_file
      ;; << file-handle -- >>
      pop_argument_stack linux64_sys_1_rdi
      mov linux64_sys_n_rax, syscall_close
      syscall
      next
   #+end_src
** read-file [linux]
...
** memory allocation
   #+begin_src fasm :tangle cicada-nymph.fasm
   size$jo_heap = 3 * 1024 * 1024 * jo_size
