...
** 記 結尾珠
   ...
   * 結尾珠 涉及到局部變元的實現方式
     1. 當一個 jojo 被入返回棧時
        ...
        和 [current_free_address$local_byte]
        三者共同組成 一個 return-point
     2. 在遇到結尾詞而退出對這次 jojo 的執行的時候
        return-point 中的 local_jo 和 local_byte
...
** 記 插 珠珠 人返回棧 到某珠珠的前面 [等效替換法 以理解]
   ...
   * before insert-jo
     | return-point-1 | local-byte-1 |
     |                | local-jo-1   |
     |                | jojo-1       |
     | return-point-2 | local-byte-2 |
     |                | local-jo-2   |
     |                | jojo-2       |
   * after
