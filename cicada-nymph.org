...
** ignore-comment
   * this is one is for
   * I have to implement comment again here
     this one is for basic-REPL
   * having to implement a function twice
     is due to the difference between the interpreter and the compiler
     the way I use to solve this problem
     is to implement the REPL as read-compile-and-execute loop
   * the only function I will implement twice in this simple example
     is comment
     other functions such as "if else then"
     can only be used by compiler
   #+begin_src fasm :tangle cicada-nymph.fasm
...
** define-function
   #+begin_src fasm
   define_function "define-function", M__define_function
      ;; << string[address, length] -- >>
      xx exit
   #+end_src
* -----------------------------------
