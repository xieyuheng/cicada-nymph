...
* some functions to write string as helper functions in assembly
* 更好的錯誤處理機制 : 找不到的時候
  * 當一個 exception 找不到自己在 return-stack 中的退出點時
    應該有更好地處理方式 而不是不處理這種情況
* change *primitive-string-area* to *string-area* and change the interface
* todo
