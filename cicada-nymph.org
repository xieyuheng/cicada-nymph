...
* todo
** update documentation to meet the explicit tail call
** syntax for exception handling in cicada-nymph
** give the end of prepare_for a name
   * do not use 0 anymore
** a version of starting forth
** more note about assembler
** add note about unique id
* ===================================
* note
** jo & jojo
...
** memory allocation in un_initialized_memory
   * implemented as a memory map
   ...
   #+end_src
** note tail call
   * the tail position of a function body must be recognized
     there are two ways
     1. recognize tail position in the threaded-code-interpreter
        #+begin_src fasm
        xx function1
        xx fucntion2
        xx end
        #+end_src
     2. recognize tail position in the compiler
        #+begin_src fasm
        xx function1
        xx tail_call, function2
        #+end_src
   * the secnod way is better
     it is explicit
** next
   #+begin_src fasm :tangle cicada-nymph.fasm
   macro next {
...
** nothing
   #+begin_src fasm :tangle cicada-nymph.fasm
   define_primitive_function "nothing", nothing
      next
   #+end_src
** ----------------------------------
** end
