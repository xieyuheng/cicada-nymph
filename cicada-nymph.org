...
** memory allocation
   #+begin_src fasm :tangle cicada-nymph.fasm
   size$primitive_string_area = 64 * 1024 ;; (byte)

   address$primitive_string_area:
      times size$primitive_string_area db 0
...
* port
** ----------------------------------
** input-port,open                    :64bit:linux:
   #+begin_src fasm :tangle cicada-nymph.fasm
   match =linux =64bit, platform machine {

   name_buffer$input_port__open labeling
      preserve 512

   define_primitive_function "input-port,open", input_port__open
      ;; << file-name-string[address, length] --
      ;;    [file handle] or [error code] >>
      pop_argument_stack rcx
      pop_argument_stack rsi
      ;; copy file-name as a null-terminal string
      mov rdi, name_buffer$input_port__open
      rep movsb
      xor rax, rax
      mov byte [rdi], al
      mov linux64_sys_2_rsi, 0 ;; OPEN__read_only
      mov linux64_sys_1_rdi, name_buffer$input_port__open
   ...
   #+end_src
** port,close                         :64bit:linux:
   #+begin_src fasm :tangle cicada-nymph.fasm
   match =linux =64bit, platform machine {

   define_primitive_function "port,close", port__close
      ;; << file-handle -- >>
      pop_argument_stack linux64_sys_1_rdi
   ...
   #+end_src
** port,read                          :64bit:linux:
   * from disk to memory
   #+begin_src fasm :tangle cicada-nymph.fasm
   match =linux =64bit, platform machine {

   define_primitive_function "port,read", port__read
...
** input-port,open                    :32bit:linux:
   ...
   define_primitive_function "input-port,open", input_port__open
      ...
      mov linux32_sys_1_ebx, name_buffer$input_port__open
...
** load-first-file
   ...
   * a function (load-file) will be implemented in core
   ...
   string$error_report__load_first_file:
      db "* (load-file) meets error  (port,read) error code : "
   .end:
