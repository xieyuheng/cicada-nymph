...
** last-jo,jo-link?
   * first jo in assembly code
   ...
   #+begin_src fasm :tangle cicada-nymph.fasm
   define_function "last-jo,jo-link?", last_jo__jo_link?
      ;; << jo -- bool >>
      xx jo_to_link
      xx fetch
      xx zero?
...
** save-into,string-area
   #+begin_src fasm :tangle cicada-nymph.fasm
   define_function "save-into,string-area", save_into__string_area
...
** note jo-link
   ...
   * from a word one can find a jo
     ...
     from source code
     it defines new function into jo-link
...
** find-jo,through-jo-link
   * find jo in jo-link by word
   #+begin_src fasm :tangle cicada-nymph.fasm
   define_variable "*first-jo,jo-link*", V__first_jo__jo_link
   ...
   define_function "find-jo,through-jo-link,loop", find_jo__through_jo_link__loop
      ...
      xx   end
      xx xswapxx
      xx dup, last_jo__jo_link?, false?branch, 5
      xx   drop, drop2
      xx   false
      xx   end
      xx jo_to_pre_jo
      xx tail_call, find_jo__through_jo_link__loop
   #+end_src
** execute-word
   #+begin_src fasm :tangle cicada-nymph.fasm
...
** save-into,jojo-area
   #+begin_src fasm :tangle cicada-nymph.fasm
   define_function "save-into,jojo-area", save_into__jojo_area
      ;; << number -- >>
...
** !undo-make-jojo
   ...
   define_exception "!undo-make-jojo", !undo_make_jojo
      ...
      xx address, V__first_jo__jo_link, save
...
** define-function
   ...
   define_function "define-function", CICADA__define_function
      ...
      xx V__first_jo__jo_link, xxswapx
      ;; << old V__current_free_address__string_area
      ;;    old V__current_free_address__jojo_area
      ;;    old V__first_jo__jo_link
      ;;    string[address, length] >>
      ...
      xx V__current_free_address__jojo_area
      xx address, V__first_jo__jo_link
      xx save
...
** *current-free-address,string-area*
   * the last_name_string_in_assembly
   ...
   #+end_src
** last_link
   * this word helps to initialize V__first_jo__jo_link
   #+begin_src fasm :tangle cicada-nymph.fasm
   last_link = link
   #+end_src
