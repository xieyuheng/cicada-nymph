...
* implement all macro by syntax-stack
  * 簡化語法擴展機制 統一使用 棧 來保存語法 以形成對語法的靈活地變更
  * make_jojo__dispatch_jo is not needed anymore
...
* -----------------------------------
* *the-story-begin*
** begin_to_interpret_threaded_code   :64bit:linux:
   #+begin_src fasm :tangle cicada-nymph.fasm
   match =linux =64bit, platform machine {
...
** note
   * for we do not build border-check
     into the interface of pop and push
     we allocation some memory below the stacks
   * the size$syntax_stack
     defines the max number of word dispatchers
   * values in syntax_stack
     are two by two
     << dispatcher [macro, predicate] >>
** memory allocation
   #+begin_src fasm :tangle cicada-nymph.fasm
   size$syntax_stack = 1024 * jo_size

      preserve 64 * jo_size
   address$syntax_stack labeling
      preserve size$syntax_stack
   #+end_src
** pointer
   #+begin_src fasm :tangle cicada-nymph.fasm
   pointer$syntax_stack:
...
** push & pop                         :64bit:
   ...
   define_primitive_function "push-syntax-stack", push_syntax_stack
      ;; argument-stack -> syntax-stack
      pop_argument_stack rax
      mov rbx, [pointer$syntax_stack]
      mov [rbx], rax
      add qword [pointer$syntax_stack], jo_size
      next

   define_primitive_function "pop-syntax-stack", pop_syntax_stack
      ;; syntax-stack -> argument-stack
      sub qword [pointer$syntax_stack], jo_size
      mov rbx, [pointer$syntax_stack]
      mov rax, [rbx]
      push_argument_stack rax
      next
...
** push & pop                         :32bit:
   ...
   define_primitive_function "push-syntax-stack", push_syntax_stack
      ;; argument-stack -> syntax-stack
      pop_argument_stack rax
      mov rsi, [pointer$syntax_stack]
      mov [rsi], rax
      add dword [pointer$syntax_stack], jo_size
      next

   define_primitive_function "pop-syntax-stack", pop_syntax_stack
      ;; syntax-stack -> argument-stack
      sub dword [pointer$syntax_stack], jo_size
      mov rsi, [pointer$syntax_stack]
      mov rax, [rsi]
      push_argument_stack rax
      next
...
** clear                              :64bit:
   ...
   define_primitive_function "clear-syntax-stack", clear_syntax_stack
      ;; << -- >>
      mov qword [pointer$syntax_stack], address$syntax_stack
      next
...
** clear                              :32bit:
   ...
   define_primitive_function "clear-syntax-stack", clear_syntax_stack
      ;; << -- >>
      mov eax, address$syntax_stack
      mov dword [pointer$syntax_stack], eax;address$syntax_stack
      next
...
** find
   #+begin_src fasm :tangle cicada-nymph.fasm
   define_function "find-syntax-stack", find_syntax_stack
      ;; << word[address, length]
      ;;    -- jo, true
      ;;    -- false >>
      xx literal, pointer$syntax_stack, fetch
      xx literal, cursor$find_syntax_stack, save
      xx find_syntax_stack__loop
      xx end

   cursor$find_syntax_stack:
      xx 0

   define_function "find-syntax-stack,loop", find_syntax_stack__loop
      ;; << word[address, length]
      ;;    -- jo, true
      ;;    -- false >>
      xx literal, cursor$find_syntax_stack, fetch
   ...
   .not_found:
      ...
      xx V__jo_size, subtraction
      xx literal, cursor$find_syntax_stack, save
      xx taca, find_syntax_stack__loop
   #+end_src
** initialize_syntax_stack
   #+begin_src fasm :tangle cicada-nymph.fasm
   define_function "initialize-syntax-stack", initialize_syntax_stack
      ;; << -- >>
      xx clear_syntax_stack
      xx literal, M__integer_string, push_syntax_stack
      xx literal, integer_string?, push_syntax_stack
      xx end
   #+end_src
* function & jojo
** ----------------------------------
...
** make-jojo,dispatch-syntax-word
   ...
   define_function "make-jojo,dispatch-syntax-word", make_jojo__dispatch_syntax_word
      ...
      ;;    string[address, length] >>
      xx dup2
      xx find_syntax_stack, true?, false?branch, 3
      xx   execute_jo
      xx   end
      xx dup2
      xx find, false?
...
** initialize-local-variable
   ...
   define_function "initialize-local-variable", initialize_local_variable
      ;; << -- >>
      xx literal, M__local_variable_save_string,  push_syntax_stack
      xx literal, local_variable_save_string?,    push_syntax_stack
      xx literal, M__local_variable_fetch_string, push_syntax_stack
      xx literal, local_variable_fetch_string?,   push_syntax_stack
      xx end
   #+end_src
** ----------------------------------
** *local-data-heap*
