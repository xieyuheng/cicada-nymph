...
* implement all macro by syntax-set
  * 簡化語法擴展機制 統一使用 棧 來保存語法 以形成對語法的靈活地變更
  * make_jojo__dispatch_jo is not needed anymore
* no macro anymore, just function with a prefix to denote that it is a macro
...
** syntax-exist?
   ...
   define_function "syntax-exist?loop", syntax_exist?loop
      ;; << predicate, function -- bool >>

      xx taca, syntax_exist?loop
   #+end_src
** drop                               :64bit:
   #+begin_src fasm :tangle cicada-nymph.fasm
   match =64bit, machine {

   define_primitive_function "drop-syntax-set", drop_syntax_set
      sub qword [pointer$syntax_set], jo_size
      next
...
** drop                               :32bit:
   ...
   define_primitive_function "drop-syntax-set", drop_syntax_set
      sub dword [pointer$syntax_set], jo_size
      next

   }
   #+end_src
** initialize_syntax_set
   #+begin_src fasm :tangle cicada-nymph.fasm
   define_function "initialize-syntax-set", initialize_syntax_set
      ;; << -- >>
      xx clear_syntax_set
      xx literal, integer_string?
      xx literal, M__integer_string
      xx add_syntax
      xx end
   #+end_src
* function & jojo
...
** make-jojo
   * 這裏又產生了特殊的一類珠
   ...
   define_function "make-jojo", make_jojo
      ...
      xx add_syntax__local_variable
      xx make_jojo__loop
      xx sub_syntax__local_variable
      xx end
...
** find
   ...
   #+end_src
** ----------------------------------
** add-syntax,local-variable
   #+begin_src fasm :tangle cicada-nymph.fasm
   define_function "add-syntax,local-variable", add_syntax__local_variable
      ;; << -- >>
      xx literal, local_variable_save_string?
      xx literal, M__local_variable_save_string
      xx add_syntax
      xx literal, local_variable_fetch_string?
      xx literal, M__local_variable_fetch_string
      xx add_syntax
      xx end
   #+end_src
** sub-syntax,local-variable
   #+begin_src fasm :tangle cicada-nymph.fasm
   define_function "sub-syntax,local-variable", sub_syntax__local_variable
      ;; << -- >>
      xx drop_syntax_set
      xx drop_syntax_set
      xx drop_syntax_set
