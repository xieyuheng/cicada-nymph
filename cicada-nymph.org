...
* 更好的錯誤處理機制 : 調用 !exception 的函數的參數傳遞
  * 目前
...
** instruction,reset-stack,exception  :64bit:
   #+begin_src fasm :tangle cicada-nymph.fasm
   match =64bit, machine {

   define_primitive_function "instruction,reset-stack,exception", reset_stack_for_exception
      ;; << -- >>
   ...
   #+end_src
** instruction,reset-stack,exception      :32bit:
   #+begin_src fasm :tangle cicada-nymph.fasm
   match =32bit, machine {
