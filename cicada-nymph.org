...
** explain$function
   ...
   * use rax as an argument
   ...
   explain$function:
      ...
      mov rbx, [V__local_memory_odd__current_free_address + jo_size]
      push_return_stack rbx
      mov rbx, [V__local_memory_even__current_free_address + jo_size]
      push_return_stack rbx
...
** offset of return-point
   #+begin_src fasm :tangle cicada-nymph.fasm
   offset__return_point$conjugate_local_memory   = (jo_size * 0)
   offset__return_point$local_memory             = (jo_size * 1)
   offset__return_point$local_variable           = (jo_size * 2)
   offset__return_point$jojo                     = (jo_size * 3)

   unit__return_point = (jo_size * 4)
   #+end_src
** *return-stack,unit*
   #+begin_src fasm :tangle cicada-nymph.fasm
...
** return-stack,insert-jo
   * 先在個循環中複製 return-stack 之後的部分
   ...
   define_primitive_function "return-stack,insert-jo", return_stack__insert_jo
      ...
      add pointer$return_stack, (jo_size * 4 * 2)
   ...
   .end:
      ...
      pop_argument_stack rax
      add rax, jo_size
      mov qword [rsi + (jo_size * 3)], rax

      mov rax, qword [rsi + (jo_size * 1)]
      mov qword [rsi + (jo_size * 4)], rax
      mov rax, qword [rsi + (jo_size * 0)]
      mov qword [rsi + (jo_size * 5)], rax
      mov rax, qword [rsi + (jo_size * 2)]
      mov qword [rsi + (jo_size * 6)], rax
      mov rax, nothing + jo_size
      mov qword [rsi + (jo_size * 7)], rax
   ...
   .end:
      ...
      pop_argument_stack rax
      add rax, jo_size
      mov dword [rsi + (jo_size * 3)], rax

      mov rax, dword [rsi + (jo_size * 1)]
      mov dword [rsi + (jo_size * 4)], rax
      mov rax, dword [rsi + (jo_size * 0)]
      mov dword [rsi + (jo_size * 5)], rax
      mov rax, dword [rsi + (jo_size * 2)]
      mov dword [rsi + (jo_size * 6)], rax
      mov rax, nothing + jo_size
      mov dword [rsi + (jo_size * 7)], rax
...
** end
   ...
   define_primitive_function "end", end
      ...
      pop_return_stack rax
      pop_return_stack rax
      mov [V__local_memory_odd__current_free_address + jo_size], rax
      next
   ...
   .return_stack_even:
      ...
      pop_return_stack rax
      pop_return_stack rax
      mov [V__local_memory_even__current_free_address + jo_size], rax
      next
...
** initialization
   ...
   define_function "initialization", initialization
      ;; << -- >>
      xx init__rule_set__jojo_compiler
      xx end
   #+end_src
** ----------------------------------
** note top-level-REPL
...
* rule-set
** 記 使用
   * rule-set,jojo-compiler 是
     語法謂詞 還有 語法函數 這種對子
     所形成的有序集合
     這樣就可以形成簡單的語境概念了
   * 每個語境都有責任
     在進入和退出時
     維護好 rule-set,jojo-compiler
...
** memory allocation
   #+begin_src fasm :tangle cicada-nymph.fasm
   size$rule_set__jojo_compiler = 1024 * jo_size

   cursor$rule_set__jojo_compiler:
      xx address$rule_set__jojo_compiler
   address$rule_set__jojo_compiler:
      times size$rule_set__jojo_compiler db 0
   #+end_src
** *rule-set,jojo-compiler*
   #+begin_src fasm :tangle cicada-nymph.fasm
   define_variable "*rule-set,jojo-compiler*", V__rule_set__jojo_compiler
      xx address$rule_set__jojo_compiler
   #+end_src
** init,rule-set,jojo-compiler
   #+begin_src fasm :tangle cicada-nymph.fasm
   define_function "init,rule-set,jojo-compiler", init__rule_set__jojo_compiler
      ;; << -- >>
      xx V__rule_set__jojo_compiler
...
** jojo-compiler
   ...
   define_function "jojo-compiler", jojo_compiler
      ...
      xx V__rule_set__jojo_compiler, push_syntax_stack
...
** allocate-local-memory
   ...
   define_primitive_function "allocate-local-memory", allocate_local_memory
      ...
      pop_argument_stack rbx
      mov rax, [V__local_memory_odd__current_free_address + jo_size]
      push_argument_stack rax
      add [V__local_memory_odd__current_free_address + jo_size], rbx
      next

   .return_stack_even:
      pop_argument_stack rbx
      mov rax, [V__local_memory_even__current_free_address + jo_size]
      push_argument_stack rax
      add [V__local_memory_even__current_free_address + jo_size], rbx
      next
